...
(provide run-genetic-solver)
(define (run-genetic-solver)
  
  (define (process-problem weights volumes costs W V C)
    
    ; Solve the problem and return (result process-data)
    (define (solve)
      ...
      (let* ((population-data (evolve (generate-first-population MAX_POPULATION) 1 1 '()))
        ...
        (list result (cdr population-data))))
    ...
    (define (draw-negative-result process-data)
      (send msg set-label "Solution not found")
      (draw-process process-data))
    
    (define (draw-positive-result weight volume cost items process-data)
    ...
    (send msg set-label "Calculating...")
    (let* ((solution (solve))
           (result (car solution))
           (process-data (cadr solution)))
      (if (eq? #f (car result))
          (draw-negative-result process-data)
          (draw-positive-result (cadr result) (caddr result) (cadddr result) (cadddr (cdr result)) process-data))))
  
  (define (delete-all-children panel)
    (define (delete-children children)
      (if (null? children)
          '()
          (begin
            (send panel delete-child (car children))
            (delete-children (cdr children)))))
    (delete-children (send panel get-children)))
  ...
  (define dialog (new dialog% [label "Enter problem"] [parent main-frame] [min-width 300] [stretchable-width #t] [stretchable-height #f]))

  (define problem-text (new text-field% [label #f] [parent dialog]))
  (new button%
       [parent dialog]
       [label "Solve"]
       [callback (lambda (button event)
                   (send dialog show #f)
                   (delete-all-children process-panel)
                   (delete-all-children items-panel)
                   (process-problem '(4 7 3 2 2 6 3 5 5 8 10 8 2 8 5 9 4) '(7 5 7 9 1 8 4 4 1 7 4 6 6 2 4 2 10) '(7 10 7 3 1 7 5 8 4 10 1 10 9 3 4 3 8) 29 20 33))])
  (send main-frame show #t)
  (send msg set-label "Enter the problem"))